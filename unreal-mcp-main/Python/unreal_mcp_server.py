...
class UnrealConnection:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.socket.settimeout(120)  # 120 second timeout for long-running commands
    ...
    def receive_full_response(self, sock, buffer_size=4096) -> bytes:
        ...
        chunks = []
        sock.settimeout(120)  # 120 second timeout for long-running commands
        try:
            while True:
    ...
    def send_command(self, command: str, params: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
        ...
        if not self.connect():
            logger.error("[Step 4] Failed to connect to Unreal Engine for command")
            return None
        ...
        try:
            ...
            logger.info(f"[Step 4a] Sending command: {command_json}")
            self.socket.sendall(command_json.encode('utf-8'))

            # Read response using improved handler
            logger.info(f"[Step 4b] Waiting for response from UE...")
            response_data = self.receive_full_response(self.socket)
            response = json.loads(response_data.decode('utf-8'))
            
            logger.info(f"[Step 4c] Complete response from Unreal: {response}")

            # Check for both error formats: {"status": "error", ...} and {"success": false, ...}
            if response.get("status") == "error":
                error_message = response.get("error") or response.get("message", "Unknown Unreal error")
                logger.error(f"[Step 4c ERROR] Unreal error (status=error): {error_message}")
            ...
            logger.error(f"[Step 4 ERROR] Error sending command: {e}")
...
_server_shutdown_event: asyncio.Event = None

def get_unreal_connection() -> Optional[UnrealConnection]:
    """Get the connection to Unreal Engine."""
    global _unreal_connection
    try:
        # Always try to reconnect for each command to avoid stale connection issues
        # This is simpler and more reliable than connection pooling
...
async def server_lifespan(server: FastMCP) -> AsyncIterator[Dict[str, Any]]:
    ...
    global _unreal_connection, _server_shutdown_event
    ...
    try:
        yield {"shutdown_event": _server_shutdown_event}
    finally:
        if _unreal_connection:
            _unreal_connection.disconnect()
            _unreal_connection = None
